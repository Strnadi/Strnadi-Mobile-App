enum DialectSummaryMode { all, aiAdmin, adminOnly }

enum SelectedDialectTier { none, confirmed, predicted, guessed }
...
List<T> selectDialectSourceParts<T>({
  ...
  if (hasSubstantiveConfirmedDialect != null) {
    final confirmedParts = allParts
        .where(hasSubstantiveConfirmedDialect)
  ...
  if (hasAuthoritativeNoDialect != null) {
    final noDialectParts = allParts
        .where(hasAuthoritativeNoDialect)
        .toList(growable: false);
...
RecordingDialectSummary summarizeRecordingDialects({
  ...
  final hasSubstantiveConfirmedSource = inputRows.any(
    (row) => row.substantiveConfirmedSource,
  );
  ...
  void addValue(String? value, List<String> output, Set<String> seen) {
