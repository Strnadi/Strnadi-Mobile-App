...
class MapBounds {
  ...
  factory MapBounds.fromJson(dynamic value) {
    ...
    final b = MapBounds(_number(j, 'north'), _number(j, 'south'),
        _number(j, 'east'), _number(j, 'west'));
    if (b.south > b.north ||
        b.south < -90 ||
        b.north > 90 ||
        b.east.abs() > 180 ||
...
class MapCluster {
  ...
  factory MapCluster.fromJson(dynamic value) {
    ...
    final dialects = _list(j, 'dialects')
        .map(MapClusterDialect.fromJson)
    ...
    final items =
        _list(j, 'items').map(MapClusterItem.fromJson).toList(growable: false);
...
class MapClustersResponse {
  ...
  factory MapClustersResponse.fromResponseData(dynamic data) {
    final j = _decode(data);
    final features =
        _list(j, 'features').map(MapCluster.fromJson).toList(growable: false);
...
String _source(Map<String, dynamic> j, {bool cluster = false}) {
  ...
  if (!['confirmed', 'ai', 'user', 'unknown', if (cluster) 'mixed']
      .contains(s)) {
...
void _uniqueItems(List<MapClusterItem> items) {
  if (items.map((i) => i.recordingId).toSet().length != items.length ||
      List.generate(items.isNotEmpty ? items.length - 1 : 0,
...
class MapClustersApiError {
  ...
  factory MapClustersApiError.fromResponse({
    ...
    final Map<String, dynamic>? nested =
        root == null ? null : _asMap(root['error']) ?? _asMap(root['problem']);
    return MapClustersApiError(
      ...
      traceId: _errorValue(
        root,
        nested,
        const <String>['traceId', 'requestId', 'correlationId'],
      ),
    );
  }
