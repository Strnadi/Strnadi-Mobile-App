...
void main() {
  group('iOS cold-start and privacy contracts', () {
    ...
    test('a cold-start link cannot bypass background and Flutter setup', () {
      ...
      final didFinishBody = appDelegate.substring(
        workmanager,
        launchReturn + 26,
      );
    ...
    test('cold-start link forwarding has a native fake-driven unit test', () {
      final helper = File(
        'ios/Runner/ColdStartLinkForwarder.swift',
      ).readAsStringSync();
      ...
      expect(
        nativeTests,
        contains('testColdStartLinkForwarderIgnoresMissingURL'),
      );
    });

    test(
      'Firebase uses delegate swizzling when no manual APNs bridge exists',
      () {
    ...
    test(
      'all permission descriptions are localized in English, Czech, German',
      () {
  ...
  group('database bootstrap source contracts', () {
    ...
    test('production adapter delegates every database phase and rethrows', () {
      final start = bootstrap.indexOf(
        'static Future<void> initializeDatabase(AppLogger logger)',
      );
    ...
    test(
      'normal app and notifications launch only after database succeeds',
      () {
        final start = main.indexOf('Future<void> runAppBootstrap() async');
        final end = main.indexOf('await AppBootstrap.runWithTelemetry(', start);
