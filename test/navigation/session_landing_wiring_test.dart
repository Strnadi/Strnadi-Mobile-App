  ...
  group('session landing wiring (source only; no API or DB)', () {
    test('navigation awaits adoption before choosing a route', () {
      ...
      final int method = source.indexOf(
        'Future<void> navigateToSessionLanding',
      );
      final int prepare = source.indexOf(
        'await prepareSessionLanding(',
        method,
      );
      final int sessionCapture = source.indexOf(
        'captureActiveVerifiedSessionId:',
        prepare,
      );
      final int adoption = source.indexOf(
    ...
    test('verified authorizator routes use centralized awaited navigation', () {
      ...
      final int checkLoggedIn = source.indexOf(
      ...
      expect(
        RegExp(
          r'await navigateToSessionLanding\(context\);',
    ...
    test(
      'only the explicit guest action directly enters LiveRec from auth',
      () {
        final RegExp liveRecorder = RegExp(r'(?:const\s+)?LiveRec\(');
        final Map<String, int> directRecorderRoutes = <String, int>{};
        for (final File file
            in Directory('lib/auth')
                .listSync(recursive: true)
                .whereType<File>()
                .where((File file) => file.path.endsWith('.dart'))) {
          final int count = liveRecorder
              .allMatches(file.readAsStringSync())
              .length;
          if (count > 0) directRecorderRoutes[file.path] = count;
        }

        expect(directRecorderRoutes, <String, int>{
          'lib/auth/authorizator.dart': 1,
        });
        final String authorizator = File(
          'lib/auth/authorizator.dart',
        ).readAsStringSync();
        final RegExpMatch directGuestRoute = liveRecorder.firstMatch(
          authorizator,
        )!;
    ...
    test(
      'LiveRec no longer races navigation with fire-and-forget adoption',
      () {
        final String source = readRecordingSources();
        final int initState = source.indexOf('void initialize()');
