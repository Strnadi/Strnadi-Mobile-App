...
void main() {
  ...
  test('recording surfaces reference keys instead of user-facing literals', () {
    final streamRec = readRecordingSources();
    final recordingForm = File(
    ...
    expect(
      streamRec,
      isNot(
        contains('Pro správné fungování aplikace je potřeba povolit mikrofon'),
      ),
    );
    expect(
      streamRec,
      isNot(contains('Pro zahájení nahrávání musíte povolit přístup k poloze')),
    );
    expect(streamRec, contains("t('streamRec.buttons.startRecording')"));
    expect(streamRec, contains("t('streamRec.buttons.pauseRecording')"));
    expect(streamRec, contains("t('streamRec.buttons.resumeRecording')"));
    ...
    expect(callback, isNot(contains(".replaceFirst('{error}', e.toString())")));
    ...
    expect(
      main,
      isNot(
        contains("t('Aplikace potřebuje povolení k mikrofonu a notifikacím."),
      ),
    );
  });

  test(
    'active Dart sources use only defined stable literal translation keys',
    () {
      final RegExp literalTranslation = RegExp(
        r'''(?<![A-Za-z0-9_.])t\(\s*(?:'([^']*)'|"([^"]*)")''',
      );
      final RegExp stableDottedKey = RegExp(
      ...
      for (final FileSystemEntity entity in Directory(
        ...
        final String activeSource = entity
            ...
            .join('\n');
        for (final RegExpMatch match in literalTranslation.allMatches(
          activeSource,
        )) {
