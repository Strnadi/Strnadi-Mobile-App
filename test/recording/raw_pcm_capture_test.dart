...
void main() {
  group('raw PCM path reservation', () {
    ...
    test(
      'fails deterministically when candidates cannot be reserved',
      () async {
        await expectLater(
  ...
  group('RawPcmCapture', () {
    test(
      'serializes writes and finish waits for the final write to drain',
      () async {
        ...
        final RawPcmCapture capture = await RawPcmCapture.start(
          reservedFile: ReservedRawPcmFile(path: 'capture.raw', writer: writer),
    ...
    test(
      'start failures close the handle but preserve the reserved file',
      () async {
        final Directory directory = await Directory.systemTemp.createTemp(
          'raw-start-failure-',
        );
    ...
    test(
      'stream failure preserves captured bytes for diagnosis or recovery',
      () async {
        final Directory directory = await Directory.systemTemp.createTemp(
          'raw-stream-failure-',
        );
    ...
    test(
      'abort drains an in-flight write and permanently blocks finalization',
      () async {
        final StreamController<List<int>> source =
            StreamController<List<int>>();
        final Completer<void> writeGate = Completer<void>();
        final _FakeWriter writer = _FakeWriter(
          writeGates: <Completer<void>>[writeGate],
        );
        final RawPcmCapture capture = await RawPcmCapture.start(
          reservedFile: ReservedRawPcmFile(path: 'capture.raw', writer: writer),
          startStream: () async => source.stream,
    ...
    test(
      'finish bounds a native stream that never closes and preserves raw',
      () async {
        final Directory directory = await Directory.systemTemp.createTemp(
          'raw-drain-timeout-',
        );
    ...
    test(
      'finish returns even when the stream cancel future never completes',
      () async {
    ...
    for (final MapEntry<String, List<int>> example in <String, List<int>>{
      'WAV': <int>[...'RIFF'.codeUnits, 36, 0, 0, 0, ...'WAVE'.codeUnits],
      'CAF': <int>[...'caff'.codeUnits, 0, 1, 0, 0],
      'AIFF': <int>[...'FORM'.codeUnits, 0, 0, 0, 4, ...'AIFF'.codeUnits],
    }.entries) {
      test(
        'rejects ${example.key} bytes instead of nesting a container',
        () async {
          final _FakeWriter writer = _FakeWriter();
          final RawPcmCapture capture = await RawPcmCapture.start(
            reservedFile: ReservedRawPcmFile(
          ...
          expect(
            writer.events.where((event) => event.startsWith('write:')),
            isEmpty,
          );
          expect(writer.events.last, 'close');
        },
    ...
    test(
      'raw bytes are wrapped once and raw input is retained until commit',
      () async {
        final Directory directory = await Directory.systemTemp.createTemp(
          'raw-finalization-',
        );
        ...
        final RawPcmCapture capture = await RawPcmCapture.start(
          reservedFile: reserved,
          startStream: () async => Stream<List<int>>.fromIterable(<List<int>>[
            <int>[1, 2],
            <int>[3, 4],
          ]),
        );
...
class _FakeWriter implements RawPcmWriter {
  ...
  _FakeWriter({this.writeGates = const <Completer<void>>[], this.failWriteAt});
