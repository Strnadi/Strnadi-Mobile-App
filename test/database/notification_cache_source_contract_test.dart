...
void main() {
  ...
  final String migrations = File(
    'lib/database/src/database_migrations.dart',
  ).readAsStringSync();
  ...
  group('notification schema v17 contract (source only; no SQLite)', () {
    test(
      'fresh databases require owner, environment, and provider id columns',
      () {
        expect(
          repository,
          matches(
            RegExp(
              r"\bopenDatabase\s*\(\s*'soundNew\.db'\s*,\s*version\s*:\s*17\s*[,)]",
            ),
          ),
        );
        expect(
          RegExp(
            r'ownerUserId TEXT NOT NULL',
    ...
    test(
      'v16 migration adds nullable scope without guessing legacy owners',
      () {
    ...
    test('the owner/environment/read lookup has a composite index', () {
      expect(
        migrations,
        contains('ON Notifications(ownerUserId, env, read, receivedAt)'),
      );
    });

    test(
      'provider deduplication is unique only within owner and environment',
      () {
        expect(
          migrations,
          contains('ON Notifications(ownerUserId, env, providerMessageId)'),
        );
        expect(migrations, contains('WHERE providerMessageId IS NOT NULL'));
      },
    );
  });
  ...
  group('notification repository ownership contract (source only)', () {
    ...
    test('list can only query the captured owner and environment', () {
      ...
      expect(
        method,
        contains('whereArgs: <Object?>[scope.ownerUserId, scope.environment]'),
      );
    ...
    test(
      'production scope requires loaded environment and activated session',
      () {
  ...
  group('background persistence contract (source only)', () {
    test(
      'background isolate loads environment and awaits scoped persistence',
      () {
    ...
    test('environment switch refreshes the newly scoped unread badge', () {
      ...
      expect(
        refreshBadge,
        contains('unawaited(DatabaseNew.refreshUnreadNotificationCount())'),
      );
    });
  });
}
