...
void main() {
  ...
  setUpAll(() {
    ...
    recordingsController = _read(
      'lib/api/controllers/recordings_controller.dart',
    );
    recordingPartsController = _read(
      'lib/api/controllers/recording_parts_controller.dart',
    );
    recordingForm = _read('lib/PostRecordingForm/RecordingForm.dart');
    incompleteUploadPrompt = _read(
      'lib/localRecordings/incomplete_upload_prompt.dart',
    );
    uploadIntegrationHelpers = _read(
      'lib/localRecordings/upload_integration_helpers.dart',
    );
    callback = _read('lib/callback_dispatcher.dart');
    backgroundTask = _read(
      'lib/database/background_recording_upload_task.dart',
    );
  });
  ...
  group('v15 static migration contract (no SQLite)', () {
    ...
    test('request-freeze columns exist before the table rebuild', () {
      ...
      final int partMarker = RegExp(
        r"'recordingParts',\s*'uploadAttempted'",
      ).allMatches(repository, upgrade).first.start;
      final int dialectMarker = RegExp(
        r"'Dialects',\s*'uploadAttempted'",
      ).allMatches(repository, upgrade).first.start;
      final int rebuild = repository.indexOf(
        '_rebuildUploadTablesForScopedBackendIds(db)',
        upgrade,
      );
    ...
    test(
      'legacy remote children regain backend parent links before rebuild',
      () {
        final int rebuild = migrations.indexOf(
          'Future<void> _rebuildUploadTablesForScoped',
        );
        final int createTables = migrations.indexOf(
          'CREATE TABLE recordings_v15',
          rebuild,
        );
        final String repair = migrations.substring(rebuild, createTables);

        final int partReverse = repair.indexOf(
          'UPDATE recordingParts SET backendRecordingId',
        );
        final int dialectReverse = repair.indexOf(
          'UPDATE Dialects SET recordingBEID',
        );
        final int partForward = repair.indexOf(
          'UPDATE recordingParts SET recordingId',
        );
        final int dialectForward = repair.indexOf(
  ...
  group('v16 capture-review migration contract (no SQLite)', () {
    test('new and upgraded databases default existing rows to reviewed', () {
      expect(
        repository,
        contains(RegExp(r"openDatabase\(\s*'soundNew.db',\s*version: 17")),
      );
      expect(
        RegExp(
      ...
      expect(v16Upgrade, contains('UPDATE recordings SET captureReviewed = 1'));
    });
  });

  group('repository concurrency and ownership contracts', () {
    test('only stale deletion leases are eligible for deletion takeover', () {
      final int claimStart = repository.indexOf(
        'static Future<_RecordingDeletionClaim>',
      );
      final int cleanupStart = repository.indexOf(
    ...
    test(
      'a deletion claim is verified before any dependent row is removed',
      () {
        final int cleanupStart = repository.indexOf(
          'static Future<void> _deleteClaimedRecordingLocally',
        );
        final int releaseStart = repository.indexOf(
          'static Future<void> _releaseDeletionClaim',
        );
        final String cleanup = repository.substring(cleanupStart, releaseStart);

        final int ownershipCheck = cleanup.indexOf(
          'WHERE id = ? AND uploadLease = ?',
        );
        final int firstChildDelete = cleanup.indexOf(
          'DELETE FROM DetectedDialects',
        );
    ...
    test('guest listing and account adoption remain environment scoped', () {
      ...
      expect(repository, contains('WHERE env = ? AND userId IS NULL'));
    });

    test('filtered backend identities are keyed by recording and part id', () {
      expect(repository, contains('Map<(int, int), int> frpBeToLocal'));
      expect(
        repository,
    ...
    test(
      'request fields stay frozen while durable remote cache paths can move',
      () {
        expect(
          repository,
          isNot(
            contains('COALESCE(uploadAttempted, 0) = 0 OR BEId IS NOT NULL'),
          ),
        );
        expect(
          repository,
        ...
        expect(
          repository,
          contains('BEId IS NOT NULL AND COALESCE(sent, 0) = 1'),
        );
        expect(
          repository,
    ...
    test('dialect request is validated, durably marked, and session-renewed '
        'before every POST', () {
      final int validation = callback.indexOf(
        'validateDialectUploadRequest(body)',
      );
      final int attempt = callback.indexOf(
        'runDialectUploadAttempt<Response<dynamic>>',
      );
    ...
    test(
      'health endpoint registration cannot veto the durable upload lease',
      () {
        final int registration = backgroundTask.indexOf(
          'healthStarted = await startHealth',
        );
        final int upload = backgroundTask.indexOf(
    ...
    test('notification failure cannot replace the upload task result', () {
      final int notifyStart = callback.indexOf(
    ...
    test('orphan part retry never bypasses aggregate upload validation', () {
      final int retryStart = repository.indexOf(
    ...
    test('legacy upload entry points delegate to the aggregate service', () {
      final int oldRecording = repositoryApi.indexOf(
        'Future<void> _sendRecording(',
      );
      final int aggregateRecording = repositoryApi.indexOf(
        'Future<void> _sendRecordingNew(',
      );
      final String recordingCompatibility = repositoryApi.substring(
        oldRecording,
        aggregateRecording,
      );
      ...
      final int oldPart = repositoryApi.indexOf(
        'Future<void> _sendRecordingPart(',
      );
      final int updateRecording = repositoryApi.indexOf(
        'Future<void> _updateRecordingBE(',
      );
      final String partCompatibility = repositoryApi.substring(
        oldPart,
        updateRecording,
      );
      expect(partCompatibility, contains('_sendRecordingNew(recording'));
      expect(partCompatibility, contains('getRecordingFromDbById'));
      expect(partCompatibility, isNot(contains('uploadRecordingPartJson')));
      expect(
        partCompatibility,
    ...
    test('upload release clears only CAS-owned transient lease state', () {
      final int release = repositoryApi.indexOf(
        'Future<void> releaseRecording(',
      );
      final int backendApi = repositoryApi.indexOf(
        'class _BackendRecordingUploadApi',
        release,
      );
      ...
      expect(
        releaseBody,
        contains('UPDATE recordingParts SET sending = 0 WHERE recordingId = ?'),
      );
    ...
    test('upload lease acquisition clears orphaned child sending flags', () {
      final int acquire = repositoryApi.indexOf(
        'Future<bool> tryAcquireRecording(',
      );
      final int renew = repositoryApi.indexOf(
        'Future<void> renewRecording(',
        acquire,
      );
      ...
      expect(acquireBody, isNot(contains('captureReviewed = 1')));
    ...
    test('unreviewed captures cannot be scheduled or backend-discovered', () {
      final int schedule = repository.indexOf(
        'static Future<void> sendRecordingBackground',
      );
      final int sendCompatibility = repository.indexOf(
        'static Future<void> sendRecording(',
        schedule,
      );
      final String scheduling = repository.substring(
        schedule,
        sendCompatibility,
      );
      final int ownerQuery = repository.indexOf(
        '_getRecordingsForOwnerSnapshot(',
      );
      final int incompleteDiscovery = repository.indexOf(
        'findIncompleteUploads({',
        ownerQuery,
      );
      final String discoveryScope = repository.substring(
        ownerQuery,
        incompleteDiscovery,
      );
      ...
      expect(discoveryScope, contains('captureReviewed = 1'));
      expect(discoveryScope, isNot(contains('COALESCE(captureReviewed, 1)')));
    });

    test(
      'draft insertion pins one explicit owner and environment snapshot',
      () {
        final int insert = repository.indexOf(
        ...
        expect(draftInsert, contains("..mail = ownerSnapshot.isGuest ? ''"));
        expect(draftInsert, contains('..userId ='));
        expect(
          draftInsert,
          contains(RegExp(r'ownerSnapshot.isGuest\s*\? null')),
        );
        expect(
          RegExp(
            '_requireRecordingOwnerSnapshotCurrent\\(ownerSnapshot\\)',
          ).allMatches(draftInsert).length,
          greaterThanOrEqualTo(2),
        );
        final int beforeTransaction = draftInsert.indexOf(
    ...
    test('ambiguous draft persistence keeps source media fail closed', () {
      ...
      expect(reconciliation, contains(RegExp(r"'mail',\s*'userId',\s*'env'")));
      expect(reconciliation, contains('ownerSnapshot: ownerSnapshot'));
      expect(
        recordingForm,
      ...
      expect(recordingForm, contains('canDeleteUnpersistedDraftFiles('));
      expect(
        recordingForm,
        contains(
    ...
    test(
      'saved draft updates pin owner, environment, and durable identity',
      () {
        final int update = repository.indexOf(
          'static Future<void> updateRecordingDraft',
        );
        final int fetchFinished = repository.indexOf(
        ...
        final int firstSessionCheck = draftUpdate.indexOf(
          '_requireRecordingOwnerSnapshotCurrent',
        );
        ...
        final int finalSessionCheck = draftUpdate.lastIndexOf(
          '_requireRecordingOwnerSnapshotCurrent',
        );
        ...
        expect(
          draftUpdate,
          contains('recordingMetadataUpdateFields(recording)'),
        );
        expect(draftUpdate, contains("..['captureReviewed'] = 1"));
        expect(draftUpdate, contains('recording.captureReviewed = true'));
        expect(draftUpdate, contains('COALESCE(parentUploadAttempted, 0) = 0'));
    ...
    test(
      'incomplete-upload discovery uses one pinned local/backend session',
      () {
        final int find = repository.indexOf('findIncompleteUploads({');
        final int resend = repository.indexOf(
          'resendMissingPartsForRecording',
          find,
        );
        final String discovery = repository.substring(find, resend);
        final int fetch = repository.indexOf(
        ...
        expect(discovery, contains('_getRecordingsForOwnerSnapshot('));
        expect(
          discovery,
        ...
        final int controllerFetch = recordingsController.indexOf(
        ...
        final String request = recordingsController.substring(
          controllerFetch,
          controllerById,
        );
    ...
    test(
      'sensitive recording calls expose expected 4xx statuses to callers',
      () {
        ...
        final String delete = methodBody(
          'deleteRecording',
          'fetchRecordingsForUser',
        );
        final String fetchById = methodBody(
          'fetchRecordingById',
          'fetchRecordingPartSummary',
        );
    ...
    test('upload reconciliation never clears deletion leases', () {
      final int reconciliation = repository.indexOf(
        'static Future<void> checkSendingRecordings',
      );
      final int clearer = repository.indexOf(
      ...
      expect(
        clear,
        contains('LOWER(TRIM(COALESCE(uploadLease, ?))) NOT LIKE ?'),
      );
      expect(clear, contains("'delete:%'"));
    });

    test('metadata PATCH is bound to a captured recording session', () {
      final int update = repositoryApi.indexOf(
        'Future<void> _updateRecordingBE(',
      );
      final int fetch = repositoryApi.indexOf(
        'Future<void> _fetchRecordingsFromBE(',
      );
      final String metadataUpdate = repositoryApi.substring(update, fetch);

      expect(metadataUpdate, contains('runWithRecordingWorkflowLease<void>'));
      expect(metadataUpdate, contains('await context.renew()'));
      expect(
        metadataUpdate,
        contains('accessToken: context.session.accessToken'),
      );
      expect(metadataUpdate, contains('host: context.session.backendHost'));
      ...
      final int controllerUpdate = recordingsController.indexOf(
        'Future<Response<dynamic>> updateRecording',
      );
      final int controllerDelete = recordingsController.indexOf(
        'Future<Response<dynamic>> deleteRecording',
      );
      final String patch = recordingsController.substring(
        controllerUpdate,
        controllerDelete,
      );
    ...
    test('backend integer aliases never truncate fractional values', () {
      ...
      final String readInt = uploadIntegrationHelpers.substring(
        parser,
        nextDeclaration,
      );
      ...
      expect(
        readInt,
        isNot(contains('if (value is num) return value.toInt()')),
      );
    });
    ...
    test('single-record fetch persists only a validated captured session', () {
      final int fetch = repositoryApi.indexOf(
        'Future<int?> _fetchRecordingFromBE(',
      );
      final String singleFetch = repositoryApi.substring(fetch);

      expect(singleFetch, contains('sessionProvider.capture()'));
      expect(singleFetch, contains('accessToken: session.accessToken'));
      expect(singleFetch, contains('host: session.backendHost'));
      expect(singleFetch, contains('environment: session.environment'));
      expect(singleFetch, contains('capturedSession: session'));
      expect(singleFetch, contains('capturedEnvironment: session.environment'));
      expect(singleFetch, contains('recordingBelongsToCapturedAccount('));
      expect(
        singleFetch,
        isNot(
          contains(
      ...
      expect(singleFetch, contains('if (responseRecordingId != id)'));

      final int request = singleFetch.indexOf(
        '_recordingsApi.fetchRecordingById',
      );
      ...
      final int persist = singleFetch.indexOf(
        'DatabaseNew.insertRecording(',
        request,
      );
      ...
      final String requestContract = recordingsController.substring(
        controllerFetch,
        summary,
      );
    ...
    test('captured insertion preserves explicit unowned backend records', () {
      ...
      expect(insertion, contains("final Object? desiredUserId ="));
      expect(insertion, contains("'mail': desiredMail"));
      expect(insertion, contains("'userId': desiredUserId"));
      expect(insertion, isNot(contains('recording.userId == null ||')));
    });

    test('sync does not repeat missing-recording cache deletion', () {
      final int sync = repository.indexOf(
        'static Future<void> syncRecordings()',
      );
      final int listing = repository.indexOf(
    ...
    test(
      'part reconciliation uses validated parent details, not a fake route',
      () {
        ...
        final String reconciliation = repositoryApi.substring(
          adapter,
          nextMethod,
        );
        ...
        expect(reconciliation, contains('_recordingsApi.fetchRecordingById('));
        ...
        expect(reconciliation, contains('recordingPayloadConfirmsPartExists('));
    ...
    test('legacy part recovery points to the real sound route securely', () {
      final int fetch = recordingPartsController.indexOf(
      ...
      final String reconciliation = recordingPartsController.substring(
        fetch,
        download,
      );
