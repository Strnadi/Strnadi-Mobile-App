...
void main() {
  group('interrupted recording review recovery (source only; no API or DB)', () {
    ...
    test(
      'unreviewed rows reopen the metadata form instead of upload details',
      () {
        final int openRecording = listSource.indexOf(
    ...
    test('recovery validates durable rows and passes the restored handoff', () {
      ...
      final int loadParts = listSource.indexOf(
        RegExp(
          r'DatabaseNew\s*\.\s*getPartsByRecordingId\(\s*recordingId\s*,?\s*\)',
        ),
        recovery,
      );
      ...
      expect(listSource, contains("t('recList.errors.reviewRecoveryFailed')"));
    });
    ...
    test('unreviewed rows are visibly labelled for review', () {
      expect(
        ...
        matches(
          RegExp(
            r"t\(\s*'recList\.status\.finishReview'\s*,?\s*\)",
            multiLine: true,
          ),
        ),
      );
