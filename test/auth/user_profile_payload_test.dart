...
void main() {
  test(
    'caches nullable metadata without retaining a previous role or nickname',
    () async {
  ...
  group('cached user profile parsing (no API or DB)', () {
    ...
    test(
      'rejects status-page text, malformed JSON, arrays, and missing names',
      () {
        for (final Object? payload in <Object?>[
          null,
          ...
          <String, Object?>{'firstName': 7, 'lastName': 'Bird'},
          <String, Object?>{
            'firstName': 'Ada',
            'lastName': 'Bird',
            'nickname': 7,
  ...
  test(
    'startup restoration awaits its loader and validates profile payload',
    () {
      ...
      expect(
        ...
        contains(
          RegExp(
            r'\bparseCachedUserProfile\s*\(\s*response\s*\.\s*data\s*,?\s*\)',
          ),
        ),
      );
