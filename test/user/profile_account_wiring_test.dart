...
void main() {
  group('profile/account production wiring (source only)', () {
    ...
    test(
      'selected photo is not made visible before successful publication',
      () {
        final String source = File('lib/user/userPage.dart').readAsStringSync();
        final int upload = source.indexOf(
          RegExp(r'_photoPublisher\s*\.\s*publishBoundedCandidate\s*\('),
        );
        final int visible = source.indexOf(
    ...
    test('connected accounts use verified activated-session ownership', () {
      final String source = File(
        'lib/user/settingsPages/connectedPlatforms.dart',
      ).readAsStringSync();
    ...
    test(
      'connected account copy is localized and failure state is retryable',
      () {
        final String source = File(
          'lib/user/settingsPages/connectedPlatforms.dart',
        ).readAsStringSync();
    ...
    test('account status controllers require explicit captured auth', () {
      final String source = File(
        'lib/api/controllers/auth_controller.dart',
      ).readAsStringSync();
      final int google = source.indexOf(
  ...
  test('source-contract tests cannot use a real API or database', () {
    final String source = File(
      'test/user/profile_account_wiring_test.dart',
    ).readAsStringSync();
